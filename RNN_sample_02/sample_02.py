...
if __name__ == '__main__':
    ...
    # データの読み込み
    data = pd.read_csv('RNN_sample_data4-3.csv')

    # 'price' カラムのデータを取得
    price_data = data['price_delta'].values

    # シーケンスデータの準備
    length_of_sequences = len(price_data)
    maxlen = 366
    ...
    for i in range(length_of_sequences - maxlen):
        x.append(price_data[i:i + maxlen])
        t.append(price_data[i + maxlen])

    x = np.array(x).reshape(-1, maxlen, 1)
    t = np.array(t).reshape(-1, 1)

    # データの形状を確認
    print(f"x shape: {x.shape}")  # (None, maxlen, 1)
    print(f"t shape: {t.shape}")  # (None, 1)
...
#plt.plot(range(len(data)), data["price"], color="gray", linestyle="--", linewidth=1, marker="o", markersize=1, markerfacecolor="black", markeredgecolor="black", label='Predicted')
plt.legend()
plt.show()
